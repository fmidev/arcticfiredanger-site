...
//         if (harvLayer !== undefined &&  map.hasLayer(harvLayer)) { map.removeLayer(harvLayer); }
...
function ndmiDateFunction() {

    let ndmiYear = sliderDate.getUTCFullYear();
    let ndmiDay = sliderDate.getUTCDate();

    let ndmiMonth1, ndmiMonth2;

    if (ndmiDay < 15) {
        ndmiMonth1 = sliderDate.getUTCMonth();
        ndmiMonth2 = sliderDate.getUTCMonth() + 1;
    }
    else {
        ndmiMonth1 = sliderDate.getUTCMonth() + 1;
        ndmiMonth2 = sliderDate.getUTCMonth() + 2;
    }

    // let ndmiDate1 = new Date(Date.UTC(ndmiYear, ndmiMonth1 - 1, ndmiDay));
    let ndmiDate2 = new Date(Date.UTC(ndmiYear, ndmiMonth2 - 1, 15));

    if (ndmiMonth1 < 10) {
        ndmiMonth1 = '0' + ndmiMonth1;
    }
    if (ndmiMonth2 < 10) {
        ndmiMonth2 = '0' + ndmiMonth2;
    }

    if (ndmiDate2 > ndmiMaxDate) {

        ndmiDateStr1 = ndmiMaxYear.toString() + ndmiMaxMonth1 + '15';
        ndmiDateStr2 = ndmiMaxYear.toString() + ndmiMaxMonth2 + '15';

    } else {

        ndmiDateStr1 = ndmiYear.toString() + ndmiMonth1 + '15';
        ndmiDateStr2 = ndmiYear.toString() + ndmiMonth2 + '15';

    }

    // console.debug(ndmiPlotDateStr, ndmiDateStr2)
...
function plotgeotiff() {
    ...
    let rasterUrl = "https://pta.data.lit.fmi.fi/sen2/s2m_ndmi/pta_sjp_s2ind_ndmi_" + ndmiDateStr1 + "_" + ndmiDateStr2 + ".tif";

    // const rasterUrl = "https://pta.data.lit.fmi.fi/sen2/s2m_ndmi/pta_sjp_s2ind_ndmi_20240815_20240915.tif";

    // const rasterUrl = "https://pta.data.lit.fmi.fi/sen2/s2m_ndmi/pta_sjp_s2ind_ndmi_20230815_20230915.tif";

    // console.debug(rasterUrl)


    // var georastercache;

    // if (ndmiStaticLayer !== undefined) {
    // // if (ndmiStaticLayer !== undefined && map.hasLayer(ndmiStaticLayer)) {
    //         map.removeLayer(ndmiStaticLayer);
    // }

    parseGeoraster(rasterUrl).then(georaster => {
        // georastercache = georaster;
        plotgeotiffstatic(georaster);
        // plotgeotiff();
    });
...
function plotgeotiffstatic(georaster) {

    ndmiPlotDateStr = ndmiDateStr2;
    ...
    if (ndmiStaticLayer !== undefined) {
        ...
        ndmiStaticLayer = new GeoRasterLayer({
            // minZoom: 10,
            // maxZoom: 12,
            georaster, pixelValuesToColorFn, resolution,
            zIndex: 10,
